...
const i18n = {
  en: {
    nav_home:'Home', nav_gallery:'Gallery', nav_about:'About', nav_testi:'Testimonials', nav_contact:'Contact',
  ...
  fr: {
    nav_home:'Accueil', nav_gallery:'Galerie', nav_about:'À propos', nav_testi:'Témoignages', nav_contact:'Contact',
    locations:'Texas · Cancún · Mexico City',
    ...
    testi2:'"Je suis repartie avec des images que je n\'aurais jamais imaginé avoir de moi-même. Le processus était respectueux, chaleureux et profondément artistique."',
    testi2_meta:'Cliente privée — New York',
    testi3:'"SENZA n\'est pas une séance photo. C\'est l\'opportunité de se voir tel qu\'on est vraiment — sans filtres, sans honte."',
    testi3_meta:'Couple — Cancún',
    testi4:'"Une expérience d\'une élégance absolue. VZ sait créer un espace où tout devient possible."',
  ...
  es: {
    nav_home:'Inicio', nav_gallery:'Galería', nav_about:'Sobre SENZA', nav_testi:'Testimonios', nav_contact:'Contacto',
    ...
    testi_note:'— Textos de ejemplo —',
    testi1:'"Una experiencia completamente diferente a cualquier cosa que había imaginado. Todo se sintió elegante, íntimo — y completamente mío."',
    ...
    testi4:'"Une expérience d\'une élégance absolue. VZ sait créer un espace où tout devient possible."',
    testi4_meta:'Cliente privée — París',
...
let lang = localStorage.getItem('senza-lang') || 'en';
...
function applyLang(l) {
  ...
  $$('.lang-btn,.mobile-lang-btn').forEach(b => b.classList.toggle('active', b.dataset.lang === l));
}
...
function heroPlay() { clearInterval(heroTimer); heroTimer = setInterval(() => heroGoto(heroIdx+1), 9000); }
...
function openLb(i) {
  lbIdx = i;
  const lb = $('#lightbox'), img = lb.querySelector('.lightbox-img');
  img.src = galleryImgs[i].src; img.alt = galleryImgs[i].alt;
...
function initLightbox() {
  ...
  galleryImgs = items.map(el => ({ src: el.querySelector('img').src, alt: el.querySelector('img').alt }));
  items.forEach((el, i) => {
  ...
  $('#lightbox')?.addEventListener('touchend', e => { const d = ts-e.changedTouches[0].clientX; if(Math.abs(d)>50) lbNav(d>0?1:-1); });
}
...
function initForm() {
  ...
  form.addEventListener('submit', e => {
    e.preventDefault();
    const t = i18n[lang]; let ok = true;
    const check = (name, key) => {
      const f = form.querySelector(`[name="${name}"]`), w = f?.closest('.form-field');
      const bad = !f?.value.trim() || (name==='email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.value));
      w?.classList.toggle('error', bad);
      if (bad) ok = false;
    };
    check('name','err_name'); check('email','err_email'); check('message','err_msg');
    if (ok) { form.style.display='none'; $('.form-success').style.display='block'; }
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  applyLang(lang);
  $$('.lang-btn,.mobile-lang-btn').forEach(b => b.addEventListener('click', () => applyLang(b.dataset.lang)));
  initHeader(); initHero(); initMobile(); initLightbox(); initForm(); initReveal();
});
